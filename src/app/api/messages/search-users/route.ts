...
export async function GET(req: NextRequest) {
  try {
    ...
    //
    // EXCEPTION: when `?includeAdmins=true` is passed, agents ALSO see
    // admin/payment_taker users. This is used by the PendingPaymentPage
    // (payment chat) so an agent can find an admin to message about
    // activating their subscription. The regular "New Message" UI does
    // NOT pass this flag, so agents still only see peers there.
    const includeAdmins = searchParams.get('includeAdmins') === 'true';
    let roleFilter: string[] | undefined;
    if (role === 'agent') {
      roleFilter = includeAdmins ? ['agent', 'admin', 'payment_taker'] : ['agent'];
    } else if (role === 'client') {
