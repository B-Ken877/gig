...
const ROLE_DASHBOARD: Record<UserRole, PageType> = {
  agent: 'agent-dashboard',
  recruiter: 'recruiter-dashboard',
  client: 'client-dashboard',
  operations: 'ops-cockpit',
  admin: 'ops-cockpit',
  visitor: 'home',
};

export default function PublicNavbar() {
  const { currentPage, navigateTo, isAuthenticated, currentUser, logout } =
    useAppStore();
    ...
    <header className="sticky top-0 z-50 w-full border-b border-[#16A34A20]">
      ...
      >
        ...
        <button
          ...
          className="transition-enterprise flex items-center gap-2.5"
        >
          <div className="flex h-9 w-9 items-center justify-center rounded-lg green-gradient">
            <span className="text-sm font-bold" style={{ color: '#0B1A2E' }}>
              GS
            </span>
          </div>
          <span
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <button
                ...
                style={{
                  color: isActive ? '#16A34A' : 'rgba(255,255,255,0.8)',
                }}
              >
                {link.label}
                {isActive && (
          ...
          {isAuthenticated && currentUser ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                <div className="px-2 py-1.5">
                  ...
                  <p className="mt-1 text-xs font-medium capitalize" style={{ color: '#16A34A' }}>
                    {currentUser.role}
                  </p>
                </div>
                <DropdownMenuSeparator />
                <DropdownMenuItem
                  onClick={() =>
                    handleNav(ROLE_DASHBOARD[currentUser.role] || 'home')
                  }
            ...
            <>
              <Button
                ...
                className="border-[#16A34A] bg-transparent text-[#16A34A] hover:bg-[#16A34A] hover:text-[#0B1A2E]"
              >
                Login
              </Button>
              <Button
                onClick={() => handleNav('register')}
                className="green-gradient font-semibold text-[#0B1A2E] hover:opacity-90 border-0"
              >
                Get Started
              </Button>
            </>
        ...
        <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
          <SheetTrigger asChild className="lg:hidden">
            <button
              className="inline-flex items-center justify-center rounded-md p-2 text-white/80 transition-enterprise hover:bg-white/10 hover:text-white"
          ...
          <SheetContent
            ...
            style={{ backgroundColor: '#0B1A2E', borderColor: '#16A34A20' }}
          >
              ...
              {NAV_LINKS.map((link) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      color: isActive ? '#16A34A' : 'rgba(255,255,255,0.75)',
              ...
              {isAuthenticated && currentUser ? (
                ...
                <div className="flex flex-col gap-3 px-4">
                  <Button
                    variant="outline"
                    onClick={() => handleNav('login')}
                    className="w-full border-[#16A34A] bg-transparent text-[#16A34A] hover:bg-[#16A34A] hover:text-[#0B1A2E]"
                  >
                    Login
                  </Button>
                  <Button
                    onClick={() => handleNav('register')}
                    className="w-full green-gradient border-0 font-semibold text-[#0B1A2E] hover:opacity-90"
                  >
                    Get Started
                  </Button>
                </div>
