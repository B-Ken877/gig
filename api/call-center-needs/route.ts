...
export async function GET(req: NextRequest) {
  try {
    // BUGFIX: Clients were seeing EVERY call center's needs on their dashboard
    // and on the "My Jobs" page. Now: if the requester is a client, scope the
    // query to their own Client record. Agents/admins still see all (so agents
    // can browse and apply).
    const auth = await getAuth(req).catch(() => null);
    const where: { isActive: boolean; clientId?: string } = { isActive: true };

    if (auth && 'userId' in auth && auth.role === 'client') {
      ...
      if (client) {
        ...
        // Client profile doesn't exist yet -> nothing to show.
